...
(function(Malcolm){
  ...
  Malcolm.db = function() {
    ...
    this.run = function(query) {
      ...
      for (var i in drivers) {
        ...
        if ('label' in query.config && drivers[i].buffer) {
          ...
          if (!buffers[label]) {
            var ttl = 1 /* hour */ * 60 /* minutes */ * 60 /* seconds */ * 1000;
            buffers[label] = new Buffer(drivers[i], query, ttl);
          }
    ...
    function Buffer(driver, query, ttl) {
      this.driver = driver;
      this.query = query;
      this.q = Q.defer();
      this.timeoutID = setTimeout(this.flush.bind(this),100);
      setTimeout(this.clear.bind(this),ttl);
    }
